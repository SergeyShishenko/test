...
* Имя файла: EXIF.РНР
*
* Описание: предоставляет функции для чтения и записи информации EXIF
* в/из сегмента приложения app1 файла в формате JPEG 
* К сожалению, поскольку данные EXIF могут распространяться в любом месте
* во всем файле изображения, а не только в одном блоке,
* невозможно передать только строку, содержащую только EXIF
* информация. Следовательно, необходимо уметь стремиться к
* любая точка в файле. Это вызывает обертки HTTP и FTP
* не работать - т. е. функции EXIF будут работать только с локальными
* файлы.
* Для работы с интернет-файлом скопируйте его локально для начала:
*
* $newfilename = tempnam ( $dir, " tmpexif" );
* копировать ( "http://whatever.com", $newfilename );
*
*
* Автор: Эван Хантер
*
...
* Пересмотр: 1.11
*
* Изменения: 1.00 - > 1.10: добавлена функция get_EXIF_TIFF, позволяющая извлекать EXIF из файла TIFF
* 1.10 - > 1.11: добавлена функциональность, позволяющая декодировать информацию XMP и Photoshop IRB
* встроенный в данные EXIF
* добавлены проверки для HTTP и ftp обертки, так как они не поддерживаются
* изменен interpret_IFD, чтобы разрешить ссылки на миниатюры работать, когда
* инструментарий переносим через каталоги
*
*
...
// TODO: тщательно протестируйте функции для записи сегментов EXIF
// Задачи : выяснить способ, чтобы позволить EXIF, чтобы нормально работать с HTTP и FTP в фантики 
// TODO: реализовать EXIF декодирование поля описания настройки устройства
// TODO: реализовать EXIF декодирование поля SpatialFrequencyResponse
// TODO: реализовать EXIF декодирование поля OECF
// TODO: реализовать EXIF декодирование поля SubjectArea
// TODO: добавление функции put_EXIF_TIFF
