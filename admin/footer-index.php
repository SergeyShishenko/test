<!-- F:\OpenServer 5.3.8\domains\localhost\www\admin\footer-index.php -->
<?php
...
$(document).ready(function() {
    ...
    $("#ChangeSubmit").click(function(e) {
        ...
        //post variables
        var clickedID = $("#id").val().split(
        "_"); //Разбиваем строку (Split работает аналогично PHP explode)
        ...
        jQuery.ajax({
            ...
            data: myData, //данные, которые будут отправлены на сервер (post переменные)
            success: function(response) {
                $("#" + $("#tbl").val() + "_" + DbNumberID).text($("#recipient-name")
            .val());
                $("#" + $("#tbl").val() + "_" + DbNumberID).parent().data('name', $(
                    "#recipient-name").val());
                // для img
                $("#image-" + DbNumberID).find('img').data('name', $("#recipient-name")
                .val());
                // для разделов
                $("#item_" + DbNumberID).find("#" + $("#tbl").val() + "-" + DbNumberID)
    ...
    $("body").on("click", "#DelSubmit", function(e) {
        ...
        if (confirm("Удалить?")) {
            ...
            var clickedID = $('input[id="id"]').val().split(
            "_"); //Разбиваем строку (Split работает аналогично PHP explode)
            var DbNumberID = clickedID[1]; //и получаем номер из массива
    ...
    $("body").on("click", " .btn-minimize.btn-round", function(e) {
        ...
        if ($target.is(':visible')) $('i', $(this)).removeClass('glyphicon-chevron-up').addClass(
    ...
    $("body").on("mouseenter", "ul.nav-tabs li", function() {
        ...
        if (tbl == 'grupp') {
            ...
            var DbNumberID = clickedID[1]; //и получаем номер из массива
            add =
                '</a> <a href="#" class="gallery-add btn" data-content="ДОБАВИТЬ ОБЪЕКТ" title="ДОБАВИТЬ ОБЪЕКТ"' +
        ...
        $(this).append(
            ...
            '</p>' +
            '</div>');
        $(this).find('.tab-controls').stop().animate({
            'margin-top': '-15'
        }, 400);
    });
    ...
    $('#myModal').on('show.bs.modal', function(event) {
        ...
        if (typeof button.data('name') === typeof undefined) {
            recipient_name = "";
        };
        ...
        if (content.indexOf("ДОБАВИТЬ") !== -1) {
            ...
            $("#ChangeSubmit").removeClass('hidden');
            $("#DelSubmit").removeClass('hidden');
        }
        ...
        $(this).find('#im').html('<img src="' + im +
            '" alt=" "class="center-block img-rounded img-thumbnail">');
...
function loadInView(files) {
    ...
    $.each(files, function(index, file) {
        ...
        var clickedEXP = files[index].name.split(
        "."); //Разбиваем строку (Split работает аналогично PHP explode)
...
function addImage(ind) {
    ...
    for (i = start; i < end; i++) {
        ...
        if ($('#dropped-files > .image').length <= maxFiles) {
            $('#dropped-files').append('<div id="img-' + i + '" class="image" style="background: url(' + dataArray[i]
                .bground + '); background-size: cover;"> <a href="#" id="drop-' + i +
                '" class="drop-button">Удалить файл</a></div>');
        }
    }
    return false;
}
    ...
    $.each(dataArray, function(index, file) {
        ...
        $.post('upload.php', dataArray[index], function(data) {
            ...
            if (dataSplit[1] == 'загружен успешно') {
                $('#uploaded-files').append('<li><a href="images/' + dataSplit[0] + '">' +
                    fileName + '</a> загружен успешно</li>');

            } else {
                $('#uploaded-files').append('<li><a href="images/' + data + '. Имя файла: ' +
                    dataArray[index].name + '</li>');
            }
