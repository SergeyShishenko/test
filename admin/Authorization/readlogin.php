...
if(isset($_POST['login']) && isset($_POST['pass'])){
    ...
    $err=check_pass($_POST['pass'],$dbconn,$err);
    ...
    if (in_array("Пользователь с таким логином существует в базе данных", $err) && count($err) == 1) {
        ...
        if($Result_user){ 
                ...
                $password = sha1(trim($_POST['pass']).$salt);       
                ...
                if ($password == $row_user['user_password']) {
                    ...
                    echo "<br> Пароль ". sha1(trim($_POST['pass']).$salt); ;
                    echo '<br> $_POST[pass] ' . trim($_POST['pass']);
